...
class SingaporeBankCode
  BANKS = ['OCBC', 'DBS', 'UOB', 'FEB', 'POSBank', 'POSBank Plus', 'HSBC', 'SC', 'CITIBANK', 'Malayan Banking Berhad']
  attr_accessor :name, :number, :bank

  def initialize(args)
    @name   = args.fetch(:name)
    @number = args.fetch(:number).to_i
    raise 'ArgumentError', "Name needs to be one of #{SingaporeBankCode::BANKS}" unless SingaporeBankCode::BANKS.include?(@name)
    @bank      = Object.const_get("Bank::" + @name.delete(' ').upcase).new
    min_length = @bank.min_length
    raise ArgumentError "Account number needs to be at least #{min_length}" unless (@number.to_s.length >= min_length)
  end
